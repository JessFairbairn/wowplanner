...
describe("WowTask ParseDate function",function(){
	beforeEach(module("wowApp"));
	...
	it("will throw exception if you try to set it as a stupid string", function(){
		expect(function(){
			WowTask.ParseDate("lol");
		}).toThrow();
	});

	it("will throw exception if you try to set it to an object", function(){		
		expect(function(){
			WowTask.ParseDate({lemon:"gothic"});
		}).toThrow();
	});
	...
	it("will correctly parse English dates", function(){		
		var task = new WowTask();
		var date = "13/2/1991";
		var result = WowTask.ParseDate(date);

		expect(result).toEqual(new Date(1991,1,13));
	});
